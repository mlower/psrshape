...
def minirms(myprofile):
    rms16 = np.zeros((16))
    seglength = int(len(myprofile)/16)
    for i in np.arange(16):
        rms16[i] = np.std(myprofile[i*seglength:(i+1)*seglength])
        #print ('RMS16 :', i, rms16[i])
    min16 = np.min(rms16)
    #print ('minimum rms: ', min16)
    return min16
...
def pulseshape(data,mysnr,mcmc):
    ...
    #Estimate the noise RMS by finding the minimum rms of the profile broken into 16 equal segmants
    noise_start = minirms(profile)
    #noise_start = np.std(profile[0:50])
